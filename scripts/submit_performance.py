...
PU = '140'
PU = '25'
PUFolder = '/nfs/dust/cms/user/beinsam/FastSim/Timing/PileUpFilesC10/'
...
'''
python scripts/submit_performance.py --slv sl6 --isfast True
python scripts/submit_performance.py --slv sl6 --isfast False
python scripts/submit_performance.py --slv sl7 --isfast True
python scripts/submit_performance.py --slv sl7 --isfast False
'''
...
def main():
	for pv in projectversion:
		...
		pufile = PUFolder+'/'+'SingleNuE10_cfi_GEN_SIM_RECOBEFMIX_DIGI_PU'+str(PU)+'.root'
		...
		script = script.replace('FASTFULL',fastfull).replace('PUFILE',pufile)
...
echo "now summary:"
grep "^MemoryCheck\|^TimeEvent>" igprofMEM_step3_config_CMSSWV_FASTFULL.log  | awk -f scripts/getTimeMemSummary.awk >> greplogs/CMSSWV_FASTFULL_timememory.txt
echo "now large grep:"
