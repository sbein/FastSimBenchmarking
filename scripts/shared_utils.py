...
def FabDraw(cGold,legs,hComponents,datamc='MC',lumi=35.9, title = '', LinearScale=False, fractionthing='(bkg-obs)/obs'):
    cGold.cd()
    leg, leg2 = legs
    ...
    if datamc=='MC':
        ...
        for ihComp, hComp in enumerate(hComponents):
            nb_before = round(hComp.GetBinContent(6),1)
            nb = round(hComp.GetBinContent(hComp.GetXaxis().GetNbins()),1)
            if ihComp<5: leg.AddEntry(hComp, hComp.GetTitle()+' ('+str(nb_before)+'->'+' '+str(nb)+'s)','lpf')
            elif ihComp<10: leg2.AddEntry(hComp, hComp.GetTitle()+' ('+str(nb_before)+'->'+str(nb)+'s)','lpf')
            else: 
                hComp.SetLineColor(kGray)
                hComp.SetFillColor(kGray)
            if ihComp==30: leg2.AddEntry(hComp,'other','lpf')
